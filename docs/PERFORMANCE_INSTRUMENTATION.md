...
## Sculpt vertex spatial hash

Sculpt metricはSpatial Hashのbuild／reuse／incremental update、radius query、falloff、Draw／Smooth／Grab処理、mutation作成、既存の全法線再計算を含む。cell sizeはmeshの平均edge lengthをbuild時だけ計算し、brush radius変更では再buildしない。

通常操作はWorkspace所有cache、Automated BenchmarkはSculpt caseローカルcacheをwarm-upから計測終了まで再利用する。topology変更または変更index不明のrevision変更はbuild、既知mutationはincremental update、同一revisionはreuseする。query不正や32,768 cell超過時はlinear fallbackする。Picking BVHとは別構造で、GPU処理ではない。Simulatorと実機を分け、実測なしに効果を断定しない。
