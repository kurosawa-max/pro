...
Debug専用RunnerはSmall／Medium／LargeごとにPicking、Draw、Smooth、Grab、Flatten、Crease、Draw X／XYZ symmetry、Crease XYZ symmetry、全法線再計算、vertex upload、index uploadを固定入力で順番に実行する。設定はwarm-up 10回、計測60回で一元管理し、warm-up値は結果へ含めない。既存Profilerの計測点を再利用するため同じ処理を二重計測しない。
...
Mesh Diagnosticsは各presetでtopology analysis、local geometry metrics、world metrics、overlay generationを独立caseとして同じwarm-up/計測回数で測る。preset結果へvertices/trianglesとoptional unique-edge contextを付けるが、既存report decodeを壊さず、固定性能threshold、Workspace history、Renderer upload、GPU完了待ちは追加しない。
...
現在は最長centroid軸の中央値分割でありSAHではない。refit後の大変形でtree品質が低下する可能性がある。GPU時間ではなくCPU時間で、Sculpt近傍検索には未使用である。Simulatorと実機の値は分け、実測なしに効果を断定しない。
