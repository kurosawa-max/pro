...
The Preview source key binds topology ID/revision, vertex revision, non-rewinding mesh and Transform versions, sanitized ObjectTransform, options, source/result counts, component and boundary metrics, total span, and analysis fingerprint. `matchesRuntimeIdentity` is intentionally lightweight: it compares runtime identity/revisions, non-rewinding versions, sanitized Transform, options, and source counts without rerunning Diagnostics or fingerprint generation during UI updates. Apply separately rebuilds the complete plan and requires both `result.estimate == preview.estimate` and fingerprint equality before mutation.

Every Preview calculation has a UUID request identity shared by the sheet coordinator and Workspace. Candidate construction is separated from model Preview installation. Only the current request may install a Preview or publish an error; an old completion cannot clear the busy state of a newer request. Axis, Count, or Spacing changes invalidate the current identity and clear both UI and model Preview. Sheet dismissal performs the same invalidation, including while a request is marked running, so a later completion cannot leave a ghost Preview. Parameter controls are disabled during Preview calculation and Apply, but request identity remains the authoritative race-safety boundary.

Sculpt, Sculpt Undo/Redo, Transform changes and Undo/Redo, all topology replacements, load, recovery, axis, Count, Spacing, and exact-state restoration after an edit make Preview stale. Camera, brush, symmetry, Face Selection content/operation, and Diagnostics visibility do not.
