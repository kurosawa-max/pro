...
previewはtopology ID/revision、vertex revision、UUID/value型の非巻戻しmesh/Transform runtime version、selection source/version、sanitized Transform、distance、解析fingerprintへ結合する。Applyは分類とestimateを再計算し、normal、adjacency、bounds、finite/index/degenerate/duplicateおよび遠方topology issue countを検証し、Picking BVHを事前構築した後だけWorkspaceへinstallする。Vertex Spatial Indexとprofiler countを更新し、Diagnosticsをstale、Face Selectionを新topologyのemptyにする。通常Renderer revision経路以外の特別分岐はない。
