...
`MeshMirror`はWorkspaceとUIへ依存しないGeometryCore境界で、現在meshをobject-local `X/Y/Z = 0`面に対して複製する。全off-plane vertexは同じ符号側を必要とし、plane crossingを切断しない。shared-edge成分ごとに、boundaryなし・seam非接触のclosed componentはdetached shellを追加し、全boundaryがseam上のdegree-2 loopであるopen half componentは元seam vertexを共有して閉じる。non-manifold、winding conflict、degenerate、duplicate、isolated、plane-only、closed-plane contact、off-plane boundary、interior seam edge/vertexを拒否する。

seam toleranceはlocal diagonalとbounded Float精度から計算し、accepted seamの対象axis値だけをexact zeroへsnapする。snapで異なるsource positionが同一点になる場合は拒否し、epsilon weldは行わない。元vertex／triangleを元順で保持し、off-plane mirror vertexを元index順に追加し、mirror triangleは`(a,c,b)`としてwindingを補正する。完成meshはboundary 0、expected component count `open + 2 × closed`、symmetry、bounds、normal、adjacency、Diagnostics invariantsをWorkspace mutation前に検証する。
