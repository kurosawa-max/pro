...
### 17.4 Sculpt Vertex Spatial Hash

Draw／Smooth／Grabの候補頂点検索にはPicking BVHとは独立したCPU Uniform Spatial Hashを使用する。WorkspaceModelがcacheを所有し、Automated BenchmarkはSculpt caseごとのcacheを持つ。cell sizeはbuild時に一意edgeの平均長から算出して0.001...1.0へclampし、degenerate meshでは0.1を使用する。

topology変更・mesh交換・変更index不明のrevision変更ではfull build、既知のSculpt mutationでは変更頂点だけbucketを移動し、同一revisionではreuseする。非有限値、bucket不整合、巨大radiusでの過大cell列挙はlinear候補検索へfallbackする。保存形式には含めない。局所法線更新と部分GPU uploadは未実装であり、全法線再計算と全vertex buffer uploadは残る。
