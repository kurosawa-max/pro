...
Transform GizmoはMove／Rotateの単一modeを持ち、初期値はMoveとする。Rotateはworld X/Y/Zを法線とする3本のringを表示し、originはobject translation、表示scaleはMoveと同じcamera距離／FOV／viewport近似を使う。ringはobject rotation／scaleに追従せず、mode・hover・active・dragは保存しない。
...
dragは開始交点の正規化vectorと現在vectorから`atan2(dot(axis, cross(start,current)), dot(start,current))`で符号付き絶対角度を計算する。内部Quaternionは`worldDelta * startRotation`で左乗算して正規化するため、既存rotationに対してもworld-space軸回転となる。translation／scaleとobject-local meshは変更せず、mesh revision／topology／GPU mesh uploadも変化しない。

Rotation ringは固定bufferを専用overlay draw callで再利用し、Moveと同じ軸色・uniform layout・depth always方針を使う。入力優先順位はactive current-mode gizmo、current-mode handle、Sculpt、Cameraである。mode切替、cancel、load、reset、Benchmark開始時はdrag開始Transformへ復元する。自由回転、screen-plane rotation、scale gizmo、local軸、snap、pivot、複数object、Transform Undo／Redoは含まない。
