...
`PlanarFaceRegion`はFace InsetとFace Bevelが共有するpure GeometryCore境界で、component／boundary分析、決定論的2D basis、strictly convex constant-width offset、local Float round-trip、inner triangulation交差安全性、deterministic mesh構築、result validationだけを所有する。Workspace transaction、preview identity、UI状態、Bevel heightは持たないため、既存Inset挙動を保ったまま共通の幾何不変条件を再利用する。

`FaceBevel`は共有offset結果の全selected duplicateをcomponent world normalへsigned heightだけ移し、outer boundaryとshifted inner boundaryの間へ2 triangles/edgeのchamfer ringを生成する。widthはpositive world millimeter、heightはsigned nonzero world millimeterで、実際のlocal Float格納値をworldへ戻してperpendicular width、signed height、`hypot(width, height)` slope、inner orientation／intersection／coverage、ring degeneracy、world boundsを検証する。inner capは元selected connectivityとwindingを維持する。strictly convexな対応boundary loopのparallel-plane ruled stripとしてringを構成し、projected crossingをoffset／inner pair検査で拒否するが、unselected geometryとの一般collision検出は行わない。

BevelはInsetと同じnon-rewinding source key、prepared／nonthrowing commit、fresh topology、full normal／adjacency／Diagnostics、Picking BVH／Spatial Index、`ReplaceMeshCommand` 1件、selection clear、Autosave record-only scopeを使う。preview／Cancel／failureはWorkspace不変で、Applyだけがproject generationを1回進める。width、height、previewは永続化せずformatVersion 1を維持する。result上限とpairwise安全上限、MainActor同期制限もInsetと共通である。general edge bevel、concave／hole／multiple-loop／non-planar region、segments、collision repair、multiple objectは含まない。詳細は`FACE_BEVEL.md`を参照する。
