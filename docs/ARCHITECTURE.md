...
### Units, dimensions, and STL import/export

Forge3Dの内部長さ1 unitは1 millimeterと定義する。mesh vertex、Primitive寸法、Transform translation、Brush radius、camera、bounds、gizmo、STL座標はこの同じ数値系を使う。既存Foundation projectの数値は変換せずmmと解釈し、formatVersion 1へunit fieldを追加しない。

`EditableMesh.bounds`はobject-local AABBである。world AABBはlocal AABBの8 cornersを`ObjectTransform.modelMatrix`で変換して再包含する。minimum/maximumの2点だけではrotation後の実寸を正しく表せないためである。

STL exportはExport境界でのみTransformをbakeし、source mesh/runtime/historyを変更しない。As Displayedはtranslationを含み、Center at Originはrotation/scale後のbounds centerを原点へ移す。non-uniform scale後も正しくするためtriangle normalは変換後の3頂点から再計算する。STL自体に正式なunit metadataはなく、headerの`unit=mm`は参考hintである。

STL importは`src/Import/STLImporter.swift`に分離する。Binaryはtriangle countとexact byte layout、ASCIIは有限状態parserで識別し、Float bit-pattern exact weld、triangle/edge検証、normal/adjacency再構築後の`EditableMesh`だけをWorkspaceへ渡す。unitは推測せず全座標をmmと解釈する。Workspace置換はidentity Transform、auto-frame camera、新runtime topologyを持つ`ReplaceMeshCommand`である。
...
2026-07-10 時点で `Forge3D.xcodeproj` に最小縦切りを実装した。論理境界は `src/App`、`UI`、`Input`、`Renderer`、`GeometryCore`、`SculptCore`、`Undo`、`FileIO`、`Import`、`Export` に分離している。
...
### 17.11 Binary and ASCII STL import
...
単一objectのFace Selectionは現在index順のtriangle番号をface IDとし、topology ID、topology revision、triangle countへ結び付けたdense UInt64 bitsetで保持する。contains／単一setはO(1)、全選択／解除／反転はstorageを再確保せずO(T/64)で、selected countは増分管理する。selection専用versionはruntime UUID identityとUInt64 valueの組で、実際の内容変更だけを記録する。value上限後は新identityへ交換してrenderer cache keyの整数wrapを防ぎ、project mutation generationとは分離する。

world RayはObjectTransformでlocalへ変換し、既存CPU BVH cacheを使う。topology変更時はbuild、vertex-only変更時はrefitし、成功後のcurrent runtime identityだけから最前面triangle IDを取得する。Face Selectのlinear scan fallbackやunchanged tapごとのBVH rebuildは行わない。Replace／Add／Remove／Toggleと、共有edge Union-Findによるconnected unionを提供し、vertex-only接触は同componentにしない。入力順はactive modal、active/current Gizmo、Face Select Pencil tap、Sculpt、finger Cameraであり、ContentViewのpresentation stateをMetalCanvasへ渡して背面入力を抑止する。

Metal selection overlayはmesh vertex bufferとmodel/view/projectionを再利用し、選択triangleのindexだけを独立bufferへ保存する。描画順はmesh、selection fill、Diagnostics、Gizmoである。非empty updateはindex検証、確保、copy成功後だけcache keyをcommitし、失敗時は古いfillを隠して同一keyを再試行する。selection/topology変更時だけbufferを更新し、camera、Transform、Sculpt vertex revisionでは更新しない。panelはsafe-area insetとadaptive gridでcompact width／Dynamic Typeへ対応する。selectionはformatVersion 1、Recovery、Autosave、Workspace historyへ保存せずdirtyを変更しない。topology置換時にclearし、vertex-only変更時は維持する。詳細は`FACE_SELECTION.md`を参照する。

`FaceExtrude`はFace Selectionを入力とするUI／Workspace／Renderer非依存のpure geometry境界である。選択faceをshared-edge componentへ分け、global useが2で反対windingのselected incident edgeだけを許可する。componentごとのDouble area vectorをworld-spaceで正規化し、signed millimeter displacementをlocal→world→inverse localでduplicate vertexへ適用する。元index順の必要vertex、component／元vertex順のduplicate、未選択face、元selected face順top、component／face／edge-slot順side wallの順に新meshを決定論的に構築する。

previewはtopology ID/revision、vertex revision、UUID/value型の非巻戻しmesh/Transform runtime version、selection source/version、sanitized Transform、distance、解析fingerprintへ結合する。Applyはmesh全体のfinite/index/degenerate/duplicate条件、分類とestimate、normal、adjacency、bounds、および遠方topology issue countを検証し、Picking BVHを事前構築した後だけWorkspaceへinstallする。throw可能なprepared phaseとnonthrowing commit phaseを分離し、Autosave snapshot許可は履歴recordの`defer` scopeだけで有効にする。Vertex Spatial Indexとprofiler countを更新し、Diagnosticsをstale、Face Selectionを新topologyのemptyにする。通常Renderer revision経路以外の特別分岐はない。Undo／RedoのBVH rebuild失敗は旧cacheを明示invalidateし、次のpickで再試行する。

Extrudeはbefore/after mesh/Transform/cameraを`ReplaceMeshCommand` 1件へ記録し、Applyだけがproject generationを1回進めて安全な完成snapshotからAutosaveをscheduleする。preview、Cancel、failureはmesh、selection、history、dirty、Recovery、runtime cache、project bytesを変更しない。formatVersion 1には通常のresult vertices/indicesだけを保存する。上限はCleanupと共有する2,000,000 vertices、4,000,000 triangles、768 MiB estimate、および1,000,000 selected facesである。初版はMainActor同期で、self-intersection、open-boundary／whole-shell extrusion、general edge bevel、multiple objectを含まない。詳細は`FACE_EXTRUDE.md`を参照する。

`FaceInset`は同じFace Selection、runtime version、preview、prepared commit、ReplaceMeshCommand境界を使うpure GeometryCore操作である。selected faceをshared-edge componentへ分け、各componentがEuler characteristic 1、exactly one oriented boundary、world-space planar、simple、strictly convexであることを検証する。area normalとleast-parallel world axisから決定論的なright-handed 2D basisを作り、CCW各edgeをpositive millimeter widthだけ左へ移動して隣接line intersectionからinner boundaryを得る。parallel／excessive miter／collapse／reversal／self-intersectionを拒否し、interior selected vertexは移動せずFloat round-trip後もinner polygonのstrict interiorに残る場合だけ許可する。

Inset結果は元index順のreferenced vertex、component／元vertex順のduplicate、未選択face、boundary loop順のring、元selected face順のinner faceで決定論的に構築する。実際のlocal Floatをworldへ戻した座標でconstant-width edge、strict interior、全triangle正向き、非隣接inner edge交差、triangle包含／same-side fold-over、area sum、preview boundsを検証する。pairwise安全検査はcomponentごとにedge／triangle各8,000,000 pairを上限とし、超過時はmutation前に拒否する。全normal/adjacency/mesh diagnostics、world/local bounds、Picking BVHをcommit前に検証し、通常Renderer topology upload経路を使う。成功はselectionをclearしDiagnostics/Cleanupをinvalidateしてproject generationを1回進め、Autosave snapshot許可を履歴record中だけ有効にする。preview／failureはWorkspaceを変更せず、formatVersion 1へoptionsやpreviewを保存しない。上限とMainActor同期制限はExtrudeと同じで、concave／hole／multiple-loop／non-planar／negative-outset／general edge bevel／multiple objectは含まない。詳細は`FACE_INSET.md`を参照する。

`PlanarFaceRegion`はFace InsetとFace Bevelが共有するpure GeometryCore境界で、component／boundary分析、決定論的2D basis、strictly convex constant-width offset、local Float round-trip、inner triangulation交差安全性、deterministic mesh構築、result validationだけを所有する。Workspace transaction、preview identity、UI状態、Bevel heightは持たないため、既存Inset挙動を保ったまま共通の幾何不変条件を再利用する。既存Insetのresult ordering、fingerprint、error分類を守るため、analysis containerと共有error名は暫定的に`FaceInset`由来のままである。第3のplanar topology consumerを追加する前に、挙動互換testを固定したうえで中立名へ一般化する。

`FaceBevel`は共有offset結果の全selected duplicateをcomponent world normalへsigned heightだけ移し、outer boundaryとshifted inner boundaryの間へ2 triangles/edgeのchamfer ringを生成する。widthはpositive world millimeter、heightはsigned nonzero world millimeterで、実際のlocal Float格納値をworldへ戻してperpendicular width、signed height、edge垂直断面の`hypot(width, height)` slope、inner orientation／intersection／coverage、ring triangleのcomponent-normal向きとsigned-height-facing、world boundsを検証する。width／height toleranceはrequested dimensionの2%以内にも制限し、large translationでminimum dimensionがFloat表現から失われる場合はmutation前に拒否する。previewとfinal boundsは同じstored local Floatとmodel matrix経路から集計するためexact equalityを要求する。inner capは元selected connectivityとwindingを維持する。strictly convexな対応boundary loopのparallel-plane ruled stripとしてringを構成し、projected crossingをoffset／inner pair検査で拒否するが、unselected geometryとの一般collision検出は行わない。

BevelはInsetと同じnon-rewinding source key、prepared／nonthrowing commit、fresh topology、full normal／adjacency／Diagnostics、Picking BVH／Spatial Index、`ReplaceMeshCommand` 1件、selection clear、Autosave record-only scopeを使う。preview／Cancel／failureはWorkspace不変で、Applyだけがproject generationを1回進める。width、height、previewは永続化せずformatVersion 1を維持する。normalは既存のarea-weighted shared-vertex再計算であり、hard-normal用vertex splitを行わないためviewportのone-segment chamferが滑らかに見える場合がある。STLは変換後triangleからfacet normalを再計算する。result上限とpairwise安全上限、MainActor同期制限もInsetと共通である。general edge bevel、concave／hole／multiple-loop／non-planar region、segments、collision repair、multiple objectは含まない。詳細は`FACE_BEVEL.md`を参照する。

### 17.19 Local-axis Mirror Copy

`MeshMirror`はWorkspaceとUIへ依存しないGeometryCore境界で、現在meshをobject-local `X/Y/Z = 0`面に対して複製する。全off-plane vertexは同じ符号側を必要とし、plane crossingを切断しない。Union-Find後にface rootを確定し、global edge列を1回だけroot別へgroup化するためcomponent解析はexpected `O(T + E)`である。shared-edge成分ごとに、boundaryなし・seam非接触のclosed componentはdetached shellを追加し、全boundaryがseam上のdegree-2 loopであるopen half componentは元seam vertexを共有して閉じる。vertex IDを共有するbow-tie edge-component、non-manifold、winding conflict、degenerate、duplicate、isolated、plane-only、closed-plane contact、off-plane boundary、interior seam edge/vertexを拒否する。

seam toleranceはlocal diagonalとbounded Float精度から計算し、accepted seamの対象axis値だけをexact zeroへsnapする。Previewはsource boundary edge総数とmaximum seam snap distanceを表示し、source key／fingerprintへ含める。snap後sourceをMirror生成前に再検証し、新規collapse／geometry duplicateを専用errorで拒否する。異なるsource positionの残る衝突も拒否し、epsilon weldは行わない。source vertex／triangle順を保持し、seam positionだけをzeroへ変更してoff-plane mirror vertexを元index順に追加し、mirror triangleは`(a,c,b)`としてwindingを補正する。完成meshはboundary 0、expected component count `open + 2 × closed`、symmetry、bounds、normal、adjacency、Diagnostics invariantsをWorkspace mutation前に検証する。

previewはtopology/revision、非巻戻しmesh/Transform version、sanitized Transform、axis、side、component/seam分類、tolerance、fingerprintへ結合する。Applyはresult meshとPicking BVHをprepared phaseで完成させ、nonthrowing commitで一度installする。`ReplaceMeshCommand` 1件、record-only Autosave snapshot scope、Face Selection clear、Diagnostics/Cleanup invalidation、Picking/Spatial Index準備、通常Renderer topology upload経路を使う。Transform、camera、tool/mode設定は維持する。formatVersion 1には通常のresult vertices/indicesだけを保存し、Mirror metadataは永続化しない。上限は2,000,000 vertices、4,000,000 triangles、768 MiBである。詳細は`MIRROR_COPY.md`を参照する。

### 17.20 Local-axis Linear Array

`MeshLinearArray`はUI／Workspace／Rendererから独立したGeometryCore境界で、現在mesh全体をobject-local X/Y/Z方向へ複製する。Countはsourceをcopy 0として含む最終数`2...256`、Spacingはsigned world-space `±0.001...1000 mm`である。`normalize(linear(ObjectTransform) × localAxis)`をworld方向とし、各copyを必ずsource world positionへ`direction × spacing × copyIndex`したDouble値から計算する。inverse Transformでlocalへ戻してFloatへ格納し、その実格納値をworldへ再変換して全vertex・全隣接copyのsigned projection、距離、垂直driftをscale／coordinate相対tolerance内で検証する。表現不能なminimum spacingはmutation前に拒否する。
...
解析はedge incidence、face/component label、component vertex set、vertex-component incidence、component boundary、全vertex exact-position incidenceを1回構築する。Splitはhost remainderとの共有がseamだけであることに加え、host外componentとのvertex-only接触を拒否する。Mergeは各位置がselected/counterpartの2頂点だけであること、counterpart loopを独立にsimple-loop検証すること、参加2componentの全vertexが各componentだけに所属すること、結果seamのincident face fanが単一であることを要求する。
...
Pickingはcurrent CPU BVHのnearest triangleを得て、その3 edgeだけをcurrent Transform／view-projectionでscreenへ写し、14-point segment distanceとedge ID tie-breakで決める。Rendererはselected／hover endpoint pairを専用bufferへuploadし、通常mesh vertex bufferから位置を読むscreen-space quadをmesh後、Diagnostics前に描画する。camera／Transform／Sculptはpair bufferを更新せず、allocation failureはoverlayだけをinvalidateして次frameのretryを許可する。詳細は`EDGE_SELECTION.md`を参照する。
