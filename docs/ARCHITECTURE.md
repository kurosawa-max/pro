...
`FaceInset`は同じFace Selection、runtime version、preview、prepared commit、ReplaceMeshCommand境界を使うpure GeometryCore操作である。selected faceをshared-edge componentへ分け、各componentがEuler characteristic 1、exactly one oriented boundary、world-space planar、simple、strictly convexであることを検証する。area normalとleast-parallel world axisから決定論的なright-handed 2D basisを作り、CCW各edgeをpositive millimeter widthだけ左へ移動して隣接line intersectionからinner boundaryを得る。parallel／excessive miter／collapse／reversal／self-intersectionを拒否し、interior selected vertexは移動せずinner polygon内に残る場合だけ許可する。

Inset結果は元index順のreferenced vertex、component／元vertex順のduplicate、未選択face、boundary loop順のring、元selected face順のinner faceで決定論的に構築する。全normal/adjacency/mesh diagnostics、world/local bounds、Picking BVHをcommit前に検証し、通常Renderer topology upload経路を使う。成功はselectionをclearしDiagnostics/Cleanupをinvalidateしてproject generationを1回進め、preview／failureはWorkspaceを変更しない。formatVersion 1へoptionsやpreviewを保存しない。上限とMainActor同期制限はExtrudeと同じで、concave／hole／multiple-loop／non-planar／negative-outset／Bevel／multiple objectは含まない。詳細は`FACE_INSET.md`を参照する。
