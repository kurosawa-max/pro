...
専用transactionはruntime mesh／selection／workspace identity、開始local／world位置、pivot、Transform、axisを固定する。Preview mesh／BVHはproject stateから分離し、commit前にcommitted BVHをprepareする。成功はsemantic `vertexRotate` command 1件で、topology、indices、ObjectTransform、selection、project formatVersion 1を維持する。詳細は`VERTEX_ROTATE.md`を参照する。
