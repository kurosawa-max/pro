...
TransformはFoundation JSON v1へ加算的に保存し、fieldがない旧dataはidentityとして読む。Transform専用UndoはStrokeHistoryの時系列再設計を避けるため本段階では未実装である。複数object、pivot、snap、Transform bake／STL適用は含まない。
...
Rendererは固定ギズモbufferをmesh後の別draw callで再利用し、origin、scale、hover／activeだけをuniform更新する。操作性を優先してdepth compareをalwaysとし、オブジェクト背後でも完全には隠さない。入力優先順位はactive gizmo、gizmo handle hit、Pencil Sculpt、指Cameraの順で、drag cancel時は開始Transformを復元する。Transform Undo／Redo、local軸、snap、pivot、複数objectは未実装である。
...
Rotation ringは固定bufferを専用overlay draw callで再利用し、Moveと同じ軸色・uniform layout・depth always方針を使う。入力優先順位はactive current-mode gizmo、current-mode handle、Sculpt、Cameraである。mode切替、cancel、load、reset、Benchmark開始時はdrag開始Transformへ復元する。自由回転、screen-plane rotation、local軸、snap、pivot、複数object、Transform Undo／Redoは含まない。
...
Move／Rotate／Scaleは相互排他で、active drag、current-mode handle、Sculpt、Cameraの入力順を維持する。mode切替、非表示化、cancel、load、reset、Benchmark開始時はScale sessionも解除し、cancelでは開始Transformを完全復元する。Benchmark中は全ギズモを非表示・操作不可とする。XY／YZ／ZX平面scale、negative scale、snapping、local/world切替、pivot、複数object、Transform Undo／Redo、mesh bakingは含まない。
