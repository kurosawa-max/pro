...
Edge BevelはEdge Selectionのvertex-disjointなmanifold interior edgeを、表示中のFloat geometryを基準としたworld-mm幅の1-segment chamferへ置換する。endpoint valence 4以上、4つの異なるsupport face、完全に分離したaffected neighborhoodを要求する。各edgeは4 offset verticesを追加し、support split child 4、strip 2、endpoint cap 2の計8 trianglesを追加してsix-edge cavityを閉じる。tetrahedronのvalence-3 cornerはmiterが必要なため拒否する。UUID Preview、prepared commit、ReplaceMeshCommand 1件を使用し、formatVersion 1には完成meshだけを保存する。詳細は`EDGE_BEVEL.md`を参照する。

## Vertex Selection foundation

Vertex Select modeはcurrent mesh vertex IDのruntime-only dense bitset、Replace／Add／Remove／Toggle、Clear／All／Invert／edge-connected selection、BVH visible-triangle picking、selected／hover point overlayを提供する。project bytes、Undo／Redo、dirty、Autosave、Recoveryへ参加せず、vertex-only編集で維持しtopology置換でclearする。詳細は`VERTEX_SELECTION.md`を参照する。
