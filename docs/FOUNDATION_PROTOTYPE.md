...
Recovery wrapperは通常project formatVersion 1を変更せず、metadata、長さ、SHA-256、project JSONを持つ。128 MiB project／160 MiB wrapper上限と空き容量確認を行い、一時fileへのwrite、synchronize、read-back validation、atomic replaceがすべて成功した後だけAutosavedへ遷移する。通常Saveはuser-selected URL、Recoveryは`Application Support/Forge3D/Recovery/current.recovery`であり、Recoveryは明示Saveの代替ではない。
