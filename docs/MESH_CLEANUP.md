...
previewは`topologyID + topologyRevision + revision`に加えて、Workspaceでmesh変更が発生するたび単調増加する非永続のmutation generationを保持する。apply直前にsource keyとestimateを再検証し、stale previewを拒否する。Undo/RedoやPrimitive、Import、Subdivision、Sculptなどで値とruntime identityがpreview時点と完全一致する状態へ戻ってもgenerationは戻らないため、古いpreviewは再有効化されない。Benchmark中と二重Cleanupは拒否する。
