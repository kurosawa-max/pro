...
Sculpt候補検索用CPU Spatial Hashとして平均edge length基準cell、topology変更時build、既知mutationのincremental update、同一revision reuse、linear fallbackを追加する。Picking BVHとは分離し、局所法線更新・部分GPU upload・GPU Computeは含めない。実測前に性能効果を断定しない。
