...
単一objectのlocal-axis Mirror Copy foundationとして、X/Y/Z zero-plane、same-side分類、closed componentのdetached duplicate、open half meshのexact seam weld、決定論的vertex/triangle順、reverse winding、必須preview、prepared commit、ReplaceMeshCommand 1件を追加する。plane crossingは切断せず、closed-plane contact、off-plane boundary、branched/incomplete seam、non-manifold／invalid sourceを拒否する。arbitrary plane、world-axis mode、live modifier、self-intersection repair、boolean union、multiple objectは後続範囲とする。
